function classes_parser(filter_name, filter_type, class_name) {
  return (
    <div key={filter_name + class_name + '_filter_option'} className='filter-element-option-container'>
      <input key={filter_type + class_name} type={filter_type === 'class_mul'? 'checkbox' : 'radio'} id={class_name} name={filter_name} value={1}></input>
...
function ClassFilter(filter_info) {
  ...
  return (
    <filedset id={filter_info.name + '_filed'} className='filter-element-container'>
